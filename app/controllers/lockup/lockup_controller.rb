  ...
  class LockupController < Lockup::ApplicationController
    ...
    def unlock
      if params[:lockup_codeword].present?
        ...
        @codeword = params[:lockup_codeword].to_s
        @return_to = params[:return_to]
        if @codeword == lockup_codeword.to_s
          set_session
          run_redirect
        end
      elsif request.post?
    ...
    def set_cookie
      cookies[:lockup] = { value: @codeword.to_s, expires: (Time.now + lockup_cookie_lifetime) }
    end

    def set_session
      session[:lockup] = @codeword.to_s
    end
